...
export const submitReport = async (req, res) => {
  try {
    const { goal: goalId, content, highlights, blockers, nextWeekPlan } = req.body;

    if (!content || content.replace(/<[^>]*>/g, '').trim().length < 10) {
      return res.status(400).json({ success: false, message: 'Report content is too short. Please write at least 10 characters.' });
    }
    ...
    const isAssigned = goal.assignedTo.some(id => id.toString() === req.user.id);
    if (!isAssigned) return res.status(403).json({ success: false, message: 'You are not assigned to this goal' });
    ...
    if (error.code === 11000) {
      return res.status(400).json({ success: false, message: 'A report for this goal already exists.' });
    }
...
export const reviewReport = async (req, res) => {
  try {
    ...
    if (!status || !['Approved', 'Revision-Required'].includes(status)) {
      return res.status(400).json({ success: false, message: 'Valid status required: Approved or Revision-Required' });
    }
...
export const summarizeReport = async (req, res) => {
  try {
    const report = await Report.findById(req.params.id).populate('goal', 'title');
    if (!report) return res.status(404).json({ success: false, message: 'Report not found' });
    ...
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a concise AI assistant that summarizes intern weekly reports for managers. Write a 2-3 sentence summary that captures the key accomplishments, blockers, and next steps. Be professional and specific.',
        },
        {
          role: 'user',
    ...
    res.status(200).json({
      success: true,
      summary: 'The intern demonstrated solid progress on the assigned task this week, completing key deliverables. Planning is clear and blockers have been identified for resolution next week.',
    });
  }
};
